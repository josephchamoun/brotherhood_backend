...
{
    $users = User::whereHas('tala2e3Roles')
        ->with('tala2e3Roles')
        ->get();
    ...
    foreach ($users as $user) {
        $hasActiveRole = $user->tala2e3Roles
            ->whereNull('end_date')
            ->count() > 0;
...
private function canManageTala2e3($user)
{
    ...
    return $user->roles()->where('section_id', 2)
                 ->whereIn('role_id', [3, 9])
                 ->whereNull('end_date')
                 ->exists();
}
...
{
    $authUser = auth()->user();
    if (!$this->canManageTala2e3($authUser)) {
...
public function activateUser(Request $request)
{
    ...
    $sectionId = $request->section_id ?? 2;
    $today = now()->toDateString();
