...
{
    $users = User::whereHas('forsanRoles')
        ->with('forsanRoles')
        ->get();
    ...
    foreach ($users as $user) {
        $hasActiveRole = $user->forsanRoles
            ->whereNull('end_date')
            ->count() > 0;
...
private function canManageForsan($user)
{
    ...
    return $user->roles()->where('section_id', 3)
                 ->whereIn('role_id', [4, 9])
                 ->whereNull('end_date')
                 ->exists();
}
...
{
    $authUser = auth()->user();
    if (!$this->canManageForsan($authUser)) {
...
public function activateUser(Request $request)
{
    ...
    $sectionId = $request->section_id ?? 3;
    $today = now()->toDateString();
